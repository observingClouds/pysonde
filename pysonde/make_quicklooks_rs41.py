...
def plot_ptrh(ds, attrs, outputpath):
    ...
    fig.suptitle('%s, %s %sUTC' % (attrs['location'],
                                   attrs['date_YYYYMMDD'],
...
def plot_map(ds, attrs, outputpath):
    ...
    # plot balloon path:
    x, y = m(ds.lon, ds.lat)
    sca = m.scatter(x,y,marker='.',c=ds.level,cmap='Reds',vmin=0.,vmax=30000.,zorder=10)
    ...
    m.plot(float(ds.lon[0]),
           float(ds.lat[0]),
           'sb', markersize=5,zorder=15)

    # and the figure title:
    ax.set_title('%s, %s %sUTC %s' % (attrs['location'], attrs['date_YYYYMMDD'],
                                      attrs['time_of_launch_HHmmss'][:-2],
                                      attrs['direction']))
